...
bot.command(:about, description: "Tells a bit about the bot") do #Tells a bit about the bot.
    "Hello.
I am **Cringe Kid**, a testing bot in discordrb created by the one and only gustav341. 
...
bot.command(:clear, description: "Syntax: .clear n\nClears out n messages. Will require something special in the future.", permission_level: 1, required_permissions: [:manage_messages]) do |event, num| #Clears n messages, including itself
...
bot.command(:ban, description: "Syntax: .ban <user> <reason>\nBans <user> for <reason>", permission_level: 1, required_permissions: [:ban_members]) do |event, mention, *reason| #Bans a mentioned member
...
bot.command(:kick, description: "Syntax: .kick <user> <reason>\nBans <user> for <reason>", permission_level: 1, required_permissions: [:kick_members]) do |event, mention, *reason| #Kicks a mentioned member
    member = event.bot.parse_mention(mention) #Converts the mention into a USER object
    ...
    event.respond reason == [] ? "Kicked #{mention}" : "Kicked #{mention} for #{reason.join(" ")}" #Informs the person that the use has been kicked
end


bot.ready do #When the bot is initalised
    bot.game = (".help") #Sets the "Playing" screen to '.help'
    bot.set_user_permission(277844547604840460, 10) #Gives my alt special powers
    bot.set_user_permission(181225342269259776, 10) #Gives yours truly god
end

bot.command(:roleid, permission_level: 1, description: "Syntax: .roleid Rolename\nReturns the role id of Rolename.") do |event, *id| #Returns the Role's id
    id = id.join(" ")
